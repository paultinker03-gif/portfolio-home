...
(() => {
  ...
  function load() {
    ...
    if (!window.omelette) { loadP = Promise.resolve((state = {})); return loadP; }
    ...
    _render() {
      ...
      if (url && !attrError) {
        ...
        if (prev !== url) {
          ...
          // An unresolved {{ … }} hole can reach here mid-stream, before
          // renderVals() substitutes it. Fetching it is a guaranteed 404.
